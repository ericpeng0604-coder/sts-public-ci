#!/usr/bin/env python3
"""Freeze reachable Anger+Pommel composition states from fresh 500xxx seeds.

The sampling contract is blind to Search/oracle quality.  Each source run adds
one normal Anger and one normal Pommel Strike before combat, ends the opening
player turn without cards, accepts only a second-turn public hand containing
exactly one of each rich card, then executes Anger followed by Pommel using only
publicly selected legal actions.  The final post-composition public state must
pass the exact formal reconstruction gate.

Source BattleContext/RNG are generation provenance only and are never exported
to Teacher observation or candidate policy state.
"""
...
SEED_FILE = ROOT / "tests" / "data" / "sts1_phase1_anger_pommel_composition_fresh_holdout_seeds_256.txt"
...
def _assert_post_composition(state: dict, *, source_seed: int) -> str:
    ...
    if state.get("energy") != 2 or state.get("block") != 0:
        raise RuntimeError(
    ...
    expected = Counter(
        {"STRIKE_RED": 5, "DEFEND_RED": 4, "BASH": 1, "POMMEL_STRIKE": 1, "ANGER": 2}
    )
    ...
    discard_pommels = sum(_card_id(card) == "POMMEL_STRIKE" for card in piles["discard_pile"])
    other_pommels = sum(
        _card_id(card) == "POMMEL_STRIKE"
        for name in ("hand", "draw_pile", "exhaust_pile")
        for card in piles[name]
    )
    ...
    if discard_pommels != 1 or other_pommels != 0:
        raise RuntimeError(
            f"composition_pommel_not_public:{source_seed}:discard={discard_pommels}:other={other_pommels}"
        )
...
def generate() -> dict:
    ...
    for source_seed in load_seeds():
        try:
            ...
            gc.obtain_card(sts.Card(sts.CardId.POMMEL_STRIKE))
            ...
            # Require both rich cards in the public second-turn hand before any
            # source action, so selection remains a simple public predicate.
            anger_action = _public_card_action(boundary_context, "ANGER")
            _public_card_action(boundary_context, "POMMEL_STRIKE")
            _execute_public_card_on_source(bc, anger_action)

            after_anger = _project(adapter, bc, run)
            after_anger_context = DecisionContext.from_public_state(after_anger)
            pommel_action = _public_card_action(after_anger_context, "POMMEL_STRIKE")
            _execute_public_card_on_source(bc, pommel_action)
            ...
            if not admitted_state.get("reconstruction", {}).get("anger_pommel_composition_complete"):
        ...
        records.append(
            {
                "case_id": f"anger-pommel-composition-fresh-v1-{len(records):02d}",
                "source_seed_provenance_only": source_seed,
                "generation_rule": (
                    "add one normal Anger and one normal Pommel Strike before combat; end opening player turn "
                    "without cards; require both in the second-turn public hand; play public Anger then public "
                    "Pommel Strike; retain only the final public projection plus bounded aggregate counters"
                ),
                "generation_public_action_ids": [anger_action.action_id, pommel_action.action_id],
    ...
    return {
        ...
        "selection_rule": (
            ...
            "500001..500256 ascending order; selection uses only public second-turn Anger+Pommel presence and "
            "formal reconstruction admission, never Search/oracle quality"
        ),
        ...
        "prior_480xxx_quality_rows_read": 0,
        "prior_490xxx_quality_rows_read": 0,
...
def main() -> None:
    ...
    print("ANGER_POMMEL_COMPOSITION_FRESH_HOLDOUT_SUITE_FREEZE = PASS")
    ...
    print("PRIOR_490XXX_QUALITY_ROWS_READ = 0")
