#!/usr/bin/env python3
"""Freeze blind, disjoint Looter turn-0 V2 discovery/holdout suites.
...
SUITES = {
    "discovery": {
        "seed_file": ROOT / "tests" / "data" / "sts1_phase1_looter_v2_discovery_seeds_64.txt",
        "min_seed": 610001,
        "max_seed": 610064,
        "schema": "sts1-phase1-looter-turn0-v2-discovery-suite-v1",
    },
    "holdout": {
        "seed_file": ROOT / "tests" / "data" / "sts1_phase1_looter_v2_fresh_holdout_seeds_64.txt",
        "min_seed": 620001,
        "max_seed": 620064,
        "schema": "sts1-phase1-looter-turn0-v2-fresh-holdout-suite-v1",
    },
}
...
def load_seeds(spec: dict) -> list[int]:
    ...
    if len(seeds) != 64 or len(set(seeds)) != 64:
        raise RuntimeError(f"looter_v2_source_seed_contract:{len(seeds)}:{len(set(seeds))}")
    if min(seeds) != spec["min_seed"] or max(seeds) != spec["max_seed"]:
        raise RuntimeError("looter_v2_source_seed_range_drift")
    if seeds != list(range(spec["min_seed"], spec["max_seed"] + 1)):
        raise RuntimeError("looter_v2_source_seed_order_drift")
    return seeds
...
def generate(kind: str) -> dict:
    ...
    for source_seed in load_seeds(spec):
        ...
        if admitted_state.get("turn") != 0:
            raise RuntimeError(f"looter_v2_opening_turn_not_zero:{source_seed}")
        enemy = admitted_state["enemies"][0]
        if enemy.get("name") != "LOOTER" or enemy.get("intent") != "LOOTER_MUG":
            raise RuntimeError(f"unexpected_opening_looter_v2:{source_seed}:{enemy}")
        ...
        records.append(
            {
                "case_id": f"looter-v2-{kind}-{len(records):02d}",
    ...
    if len(records) != TARGET_CASES:
        raise RuntimeError(
            f"insufficient_unique_admitted_looter_v2_cases:{kind}:{len(records)}/{TARGET_CASES}:rejected={rejected}"
        )
...
def main() -> None:
    ...
    print(f"LOOTER_V2_{args.suite_kind.upper()}_SUITE_FREEZE = PASS")
