#!/usr/bin/env python3
"""Freeze the first Cultist public-search V1 quality suite.

Selection is blind to Search/oracle quality: scan the separately precommitted
Cultist source seeds in file order and keep the first 24 unique opening Cultist
public decision signatures that pass the exact V1 reconstruction admission gate.
...
SEED_FILE = ROOT / "tests" / "data" / "sts1_phase1_public_search_v1_cultist_source_seeds_64.txt"
SUITE_SCHEMA = "sts1-phase1-public-search-v1-cultist-quality-suite-v1"
...
def load_seeds() -> list[int]:
    ...
    if len(seeds) != EXPECTED_SOURCE_SEEDS or len(set(seeds)) != EXPECTED_SOURCE_SEEDS:
        raise RuntimeError(f"cultist_source_seed_contract:{len(seeds)}:{len(set(seeds))}")
    return seeds
...
def generate() -> dict:
    ...
    for source_seed in load_seeds():
        ...
        # This source BattleContext is used only to generate candidate public
        # observations. Formal Search never receives or clones it.
        ...
        if admitted_state.get("turn") != 0:
            raise RuntimeError(f"cultist_opening_turn_not_zero:{source_seed}:{admitted_state.get('turn')}")
        enemy = admitted_state["enemies"][0]
        if enemy.get("name") != "CULTIST" or enemy.get("intent") != "CULTIST_INCANTATION":
            raise RuntimeError(f"unexpected_opening_cultist:{source_seed}:{enemy}")
        ...
        records.append(
            {
                "case_id": f"cultist-v1-{len(records):02d}",
    ...
    if len(records) != TARGET_CASES:
        raise RuntimeError(
            f"insufficient_unique_admitted_cultist_cases:{len(records)}/{TARGET_CASES}:rejected={rejected}"
        )
    ...
    return {
        ...
        "selection_rule": "first 24 unique fully V1-admitted opening Cultist public decision signatures in committed Cultist source-seed order",
...
def main() -> None:
    ...
    print("PUBLIC_SEARCH_V1_CULTIST_SUITE_FREEZE = PASS")
    ...
    print("CULTIST_QUALITY_RESULTS_OBSERVED_DURING_SELECTION = 0")
