#!/usr/bin/env python3
"""Freeze the first Gremlin Nob public-search V1 quality suite.

Selection is blind to Search/oracle quality: scan the precommitted Nob source
seeds in file order and keep the first 24 unique opening Gremlin Nob public
...
SEED_FILE = ROOT / "tests" / "data" / "sts1_phase1_public_search_v1_gremlin_nob_source_seeds_64.txt"
SUITE_SCHEMA = "sts1-phase1-public-search-v1-gremlin-nob-quality-suite-v1"
...
def load_seeds() -> list[int]:
    ...
    if len(seeds) != EXPECTED_SOURCE_SEEDS or len(set(seeds)) != EXPECTED_SOURCE_SEEDS:
        raise RuntimeError(f"gremlin_nob_source_seed_contract:{len(seeds)}:{len(set(seeds))}")
    return seeds
...
def generate() -> dict:
    ...
    for source_seed in load_seeds():
        ...
        # Source context is only a public-observation generator. Formal Search
        # never receives or clones it.
        ...
        if admitted_state.get("turn") != 0:
            raise RuntimeError(f"gremlin_nob_opening_turn_not_zero:{source_seed}:{admitted_state.get('turn')}")
        enemy = admitted_state["enemies"][0]
        if enemy.get("name") != "GREMLIN_NOB" or enemy.get("intent") != "GREMLIN_NOB_BELLOW":
            raise RuntimeError(f"unexpected_opening_gremlin_nob:{source_seed}:{enemy}")
        ...
        records.append(
            {
                "case_id": f"gremlin-nob-v1-{len(records):02d}",
    ...
    if len(records) != TARGET_CASES:
        raise RuntimeError(
            f"insufficient_unique_admitted_gremlin_nob_cases:{len(records)}/{TARGET_CASES}:rejected={rejected}"
        )
    ...
    return {
        ...
        "selection_rule": "first 24 unique fully V1-admitted opening Gremlin Nob public decision signatures in committed source-seed order",
...
def main() -> None:
    ...
    print("PUBLIC_SEARCH_V1_GREMLIN_NOB_SUITE_FREEZE = PASS")
    ...
    print("GREMLIN_NOB_QUALITY_RESULTS_OBSERVED_DURING_SELECTION = 0")
