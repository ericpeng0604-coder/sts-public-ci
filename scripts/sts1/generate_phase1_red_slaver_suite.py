#!/usr/bin/env python3
"""Freeze disjoint Red Slaver turn-0 discovery/fresh-holdout suites.
...
Red Slaver turn-0 public decision signatures. Source seeds are provenance only.
"""
...
SUITES = {
    "discovery": {
        "seed_file": ROOT / "tests" / "data" / "sts1_phase1_red_slaver_discovery_seeds_64.txt",
        "min_seed": 560001,
        "max_seed": 560064,
        "schema": "sts1-phase1-red-slaver-turn0-discovery-suite-v1",
    },
    "holdout": {
        "seed_file": ROOT / "tests" / "data" / "sts1_phase1_red_slaver_fresh_holdout_seeds_64.txt",
        "min_seed": 570001,
        "max_seed": 570064,
        "schema": "sts1-phase1-red-slaver-turn0-fresh-holdout-suite-v1",
    },
}
...
def load_seeds(spec: dict) -> list[int]:
    ...
    if len(seeds) != 64 or len(set(seeds)) != 64:
        raise RuntimeError(f"red_slaver_source_seed_contract:{len(seeds)}:{len(set(seeds))}")
    if min(seeds) != spec["min_seed"] or max(seeds) != spec["max_seed"]:
        raise RuntimeError("red_slaver_source_seed_range_drift")
    return seeds
...
def generate(kind: str) -> dict:
    ...
    for source_seed in load_seeds(spec):
        ...
        bc.init_encounter(gc, sts.MonsterEncounter.RED_SLAVER)
        ...
        if admitted_state.get("turn") != 0:
            raise RuntimeError(f"red_slaver_opening_turn_not_zero:{source_seed}")
        enemy = admitted_state["enemies"][0]
        if enemy.get("name") != "RED_SLAVER" or enemy.get("intent") != "RED_SLAVER_STAB":
            raise RuntimeError(f"unexpected_opening_red_slaver:{source_seed}:{enemy}")
        ...
        records.append(
            {
                "case_id": f"red-slaver-{kind}-v1-{len(records):02d}",
    ...
    if len(records) != TARGET_CASES:
        raise RuntimeError(
            f"insufficient_unique_admitted_red_slaver_cases:{kind}:{len(records)}/{TARGET_CASES}:rejected={rejected}"
        )
    ...
    return {
        ...
        "selection_rule": f"first 24 unique fully admitted Red Slaver turn-0 public decision signatures in committed {spec['min_seed']}..{spec['max_seed']} source-seed order",
...
def main() -> None:
    ...
    print(f"RED_SLAVER_{args.suite_kind.upper()}_SUITE_FREEZE = PASS")
