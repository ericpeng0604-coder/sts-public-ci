#!/usr/bin/env python3
"""Evaluate the already-frozen Anger+Finesse candidate on 540xxx fresh holdout.

No tuning is permitted here. Candidate code and Search/oracle budgets are reused
unchanged from the frozen 530 discovery contract; this file only evaluates new
540 public cases and emits the holdout verdict.
"""
...
from scripts.sts1.run_phase1_anger_finesse_composition_discovery import oracle_scores_for_case

SCHEMA = "sts1-phase1-anger-finesse-composition-fresh-holdout-v1"
...
HOLDOUT_GATE_ID = "anger-finesse-fresh-holdout-candidate-ge-raw-gt-simple-random-v1"
...
def run_once(suite: dict) -> tuple[list[dict], dict]:
    ...
    for record in suite["cases"]:
        ...
        search_result = PublicStateSearch(__import__(
            "roguelike_ai.sts1_teacher.sampling", fromlist=["PairedPublicSampleEvaluator"]
        ).PairedPublicSampleEvaluator(backend), config).run(context)
        ...
        # This function is code-frozen from discovery and therefore preserves
        # the exact 4 public samples x 2000 native MCTS sims/action oracle.
    ...
    summary = {
        ...
        "holdout_gate_id": HOLDOUT_GATE_ID,
        ...
        "illegal": illegal,
        "oracle_samples": 4,
        "oracle_mcts_sims_per_action_per_sample": 2000,
        ...
        "fresh_holdout_verdict": "PASS" if not reasons else "FAIL",
        "fresh_holdout_reasons": reasons,
...
def main() -> None:
    ...
    if suite.get("purpose") != "FRESH_HOLDOUT_ONLY":
        raise RuntimeError("fresh_holdout_purpose_missing")
    if suite.get("quality_results_observed_during_selection") != 0:
        raise RuntimeError("holdout_selection_was_not_blind")
    if suite.get("discovery_decision_rows_read") != 0 or suite.get("prior_530xxx_quality_rows_read") != 0:
        raise RuntimeError("discovery_quality_leaked_into_holdout_selection")
    for key in (
        "prior_500xxx_quality_rows_read", "prior_510xxx_quality_rows_read", "prior_520xxx_quality_rows_read"
    ):
        if suite.get(key) != 0:
    ...
    if rows1 != rows2 or summary1 != summary2:
        raise RuntimeError("anger_finesse_fresh_holdout_not_deterministic")
    ...
    print("ANGER_FINESSE_FRESH_HOLDOUT_EXECUTION = PASS")
    print(f"ANGER_FINESSE_FRESH_HOLDOUT_VERDICT = {final['fresh_holdout_verdict']}")
