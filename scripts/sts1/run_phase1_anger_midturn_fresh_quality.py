#!/usr/bin/env python3
"""Run the predeclared fresh quality gate for reachable Anger midturn states."""
...
from roguelike_ai.sts1_teacher.anger_midturn_policy_v1 import (
    MARGIN_THRESHOLD,
    POLICY_ID,
...
SCHEMA = "sts1-phase1-anger-midturn-fresh-quality-v1"
...
# Frozen before any 480xxx quality result:
# candidate >= Raw Search, candidate > Simple, candidate > Random,
# unresolved=0, exact two-repeat determinism, exactly 24 fresh admitted cases.
GATE_ID = "anger-midturn-fresh-v1-candidate-ge-search-gt-simple-random"
...
def run_once(suite: dict) -> tuple[list[dict], dict]:
    ...
    for record in suite["cases"]:
        ...
        candidate = choose_anger_midturn_candidate(context, search_result, simple.action_id)
    ...
    summary = {
        "schema_version": SCHEMA,
        "scope": "reachable Jaw Worm second-player-turn post-Anger public states only",
...
def main() -> None:
    ...
    if rows1 != rows2 or summary1 != summary2:
        raise RuntimeError("anger_midturn_fresh_quality_not_deterministic")
    ...
    print("ANGER_MIDTURN_FRESH_QUALITY_EXECUTION = PASS")
    print(f"ANGER_MIDTURN_FRESH_QUALITY_VERDICT = {final['quality_verdict']}")
