#!/usr/bin/env python3
"""Evaluate frozen V2 vs V1/Search/Simple/Random on 510xxx discovery only."""
...
SCHEMA = "sts1-phase1-anger-pommel-composition-v2-discovery-v1"
...
DISCOVERY_GATE_ID = "anger-pommel-v2-discovery-ge-raw-v1-gt-simple-random"
...
def run_once(suite: dict) -> tuple[list[dict], dict]:
    ...
    names = ("v2", "v1", "raw_search", "simple", "random")
    ...
    for record in suite["cases"]:
        ...
        v1 = choose_anger_pommel_composition_candidate(context, search_result, simple.action_id)
        v2 = choose_anger_pommel_composition_candidate_v2(context, search_result, simple.action_id)
        v1_fallbacks += int(v1.used_simple_fallback)
        v2_failsafes += int(v2.used_simple_failsafe)
        v2_top_tie_breaks += int(v2.used_simple_top_tie_break)
        top_semantic_counts[v2.top_semantic_count] += 1
        ...
        agreement = {
            "v2": single_action_agreement(v2.action_id, oracle_tie_ids),
            "v1": single_action_agreement(v1.action_id, oracle_tie_ids),
        ...
        rows.append(
            {
                ...
                "v2_used_simple_failsafe": v2.used_simple_failsafe,
                "v2_used_simple_top_tie_break": v2.used_simple_top_tie_break,
                "v2_top_semantic_count": v2.top_semantic_count,
                "v1_action_id": v1.action_id,
                "v1_used_simple_fallback": v1.used_simple_fallback,
    ...
    if counts["v2"] < counts["v1"]:
        reasons.append("v2_worse_than_v1")
    ...
    summary = {
        "schema_version": SCHEMA,
        "purpose": "V2_DISCOVERY_ONLY_NOT_HOLDOUT",
        "discovery_gate_id": DISCOVERY_GATE_ID,
        "v2_policy_id": V2_POLICY_ID,
        "v1_policy_id": V1_POLICY_ID,
        "case_count": total,
        "agreement_counts": counts,
        "agreement_rates": {name: counts[name] / total for name in names},
        "v2_simple_failsafe_count": v2_failsafes,
        "v2_simple_top_tie_break_count": v2_top_tie_breaks,
        "v2_top_semantic_count_histogram": dict(sorted(top_semantic_counts.items())),
        "v1_simple_fallback_count": v1_fallbacks,
...
def main() -> None:
    ...
    if suite.get("purpose") != "V2_DISCOVERY_ONLY_NOT_HOLDOUT":
    ...
    if suite.get("prior_500xxx_quality_rows_read") != 0:
        raise RuntimeError("v1_holdout_rows_leaked_into_discovery")

    rows1, summary1 = run_once(suite)
    rows2, summary2 = run_once(suite)
    if rows1 != rows2 or summary1 != summary2:
        raise RuntimeError("composition_v2_discovery_not_deterministic")
    ...
    print("ANGER_POMMEL_V2_DISCOVERY_EXECUTION = PASS")
    print(f"ANGER_POMMEL_V2_DISCOVERY_VERDICT = {final['discovery_verdict']}")
