#!/usr/bin/env python3
"""Evaluate blind pristine Cultist turn-1 suites with the existing public Search."""
...
from roguelike_ai.sts1_teacher.native_rollout import NativePublicCultistRolloutV1
...
CANDIDATE_ID = "cultist-turn1-pristine-public-search-v1"
ORACLE_MCTS_SIMS = 2000
ORACLE_SAMPLE_COUNT = 4
SEARCH_SAMPLE_COUNT = 8
SEARCH_SAMPLING_SEED = 20260831
EXPECTED_SCHEMAS = {
    "discovery": "sts1-phase1-cultist-turn1-pristine-discovery-suite-v1",
    "holdout": "sts1-phase1-cultist-turn1-pristine-fresh-holdout-suite-v1",
}
...
def run_once(suite: dict, suite_kind: str) -> tuple[list[dict], dict]:
    ...
    for record in suite["cases"]:
        ...
        if context.decision_signature != record["decision_signature"]:
            raise RuntimeError(f"cultist_turn1_suite_signature_drift:{record['case_id']}")
        enemy = state["enemies"][0]
        if (
            state.get("turn") != 1
            or enemy.get("name") != "CULTIST"
            or enemy.get("intent") != "CULTIST_DARK_STRIKE"
            or enemy.get("powers") != []
            or enemy.get("hp") != enemy.get("max_hp")
        ):
            raise RuntimeError(f"cultist_turn1_suite_scope_drift:{record['case_id']}")

        backend = NativePublicCultistRolloutV1(state, sts)
        if backend.uses_hidden_information:
            raise RuntimeError("cultist_turn1_backend_hidden_information")
        ...
        if simple is None or random_action is None:
            raise RuntimeError(f"cultist_turn1_baseline_missing_action:{record['case_id']}")
        ...
        rows.append(
            {
                "schema_version": f"sts1-phase1-cultist-turn1-pristine-{suite_kind}-quality-v1",
    ...
    summary = {
        ...
        "candidate_id": CANDIDATE_ID,
        "candidate_policy_origin": "unchanged NativePublicCultistRolloutV1",
...
def main() -> None:
    ...
    if not hasattr(sts, "get_public_monster_ritual_v1"):
        raise RuntimeError("cultist_ritual_proof_binding_missing")
    ...
    if (
        ...
        or suite.get("quality_results_observed_during_selection") != 0
    ):
        raise RuntimeError("cultist_turn1_suite_freeze_contract_failed")
    ...
    if rows1 != rows2 or summary1 != summary2:
        raise RuntimeError("cultist_turn1_quality_not_deterministic")
    ...
    print(f"CULTIST_TURN1_{args.suite_kind.upper()}_QUALITY_EXECUTION = PASS")
    print(f"CULTIST_TURN1_{args.suite_kind.upper()}_QUALITY_VERDICT = {final_summary['quality_verdict']}")
    print(f"UNRESOLVED = {final_summary['search_unresolved']}")
    print("ILLEGAL = 0")
    print("LEAKAGE = 0")
