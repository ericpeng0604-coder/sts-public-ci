#!/usr/bin/env python3
"""Evaluate frozen Looter turn-0 V2 suites with bounded gold tie-break Search."""
...
CANDIDATE_ID = "looter-turn0-public-search-gold-tiebreak-v2"
ORACLE_MCTS_SIMS = 2000
ORACLE_SAMPLE_COUNT = 4
SEARCH_SAMPLE_COUNT = 8
SEARCH_SAMPLING_SEED = 20260830
EXPECTED_SCHEMAS = {
    "discovery": "sts1-phase1-looter-turn0-v2-discovery-suite-v1",
    "holdout": "sts1-phase1-looter-turn0-v2-fresh-holdout-suite-v1",
}
...
def frozen_search_config() -> SearchConfig:
    ...
    if NativePublicLooterRolloutV2.gold_tiebreak_cap >= combat_mean_min_nonzero_gap:
        raise RuntimeError("looter_v2_gold_cap_can_overturn_combat_score")
    if NativePublicLooterRolloutV2.gold_tiebreak_per_gold <= config.tie_tolerance:
        raise RuntimeError("looter_v2_one_gold_cannot_break_exact_search_tie")
    return config
...
def run_once(suite: dict, suite_kind: str) -> tuple[list[dict], dict]:
    ...
    for record in suite["cases"]:
        ...
        if context.decision_signature != record["decision_signature"]:
            raise RuntimeError(f"looter_v2_suite_signature_drift:{record['case_id']}")
        ...
        if (
            ...
            raise RuntimeError(f"looter_v2_suite_scope_drift:{record['case_id']}")

        backend = NativePublicLooterRolloutV2(state, sts)
        if backend.uses_hidden_information:
            raise RuntimeError("looter_v2_backend_hidden_information")
        ...
        if simple is None or random_action is None:
            raise RuntimeError(f"looter_v2_baseline_missing_action:{record['case_id']}")
        ...
        rows.append(
            {
                "schema_version": f"sts1-phase1-looter-turn0-v2-{suite_kind}-quality-v1",
    ...
    summary = {
        ...
        "case_count": total,
        "search_config": {
            "samples_per_semantic_action": config.samples_per_semantic_action,
        ...
        "gold_tiebreak": {
            "definition": "combat_score + min(max(remaining_gold,0)*1e-7,0.001)",
            "per_gold": NativePublicLooterRolloutV2.gold_tiebreak_per_gold,
            "cap": NativePublicLooterRolloutV2.gold_tiebreak_cap,
            "frozen_combat_mean_min_nonzero_gap": 1.0 / SEARCH_SAMPLE_COUNT,
            "can_overturn_nonzero_combat_mean_gap": False,
        },
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("looter_v2_suite_freeze_contract_failed")
    ...
    if rows1 != rows2 or summary1 != summary2:
        raise RuntimeError("looter_v2_quality_not_deterministic")
    ...
    print(f"LOOTER_V2_{args.suite_kind.upper()}_QUALITY_EXECUTION = PASS")
    print(f"LOOTER_V2_{args.suite_kind.upper()}_QUALITY_VERDICT = {final_summary['quality_verdict']}")
