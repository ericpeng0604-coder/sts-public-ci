#!/usr/bin/env python3
"""Run the frozen opening-Blue-Slaver quality benchmark for PublicStateSearch V1."""
...
from roguelike_ai.sts1_teacher.native_rollout import NativePublicBlueSlaverRolloutV1
...
SCHEMA = "sts1-phase1-public-search-v1-blue-slaver-quality-v1"
EXPECTED_SUITE_SCHEMA = "sts1-phase1-public-search-v1-blue-slaver-quality-suite-v1"
...
def run_once(suite: dict) -> tuple[list[dict], dict]:
    ...
    by_intent = {intent: {"cases": 0, "search": 0, "simple": 0, "random": 0} for intent in sorted(_ALLOWED_INTENTS)}
    unresolved = 0
    ...
    for record in suite["cases"]:
        ...
        if context.decision_signature != record["decision_signature"]:
            raise RuntimeError(f"blue_slaver_suite_signature_drift:{record['case_id']}")
        enemy = state["enemies"][0]
        intent = enemy.get("intent")
        if state.get("turn") != 0 or enemy.get("name") != "BLUE_SLAVER" or intent not in _ALLOWED_INTENTS:
            raise RuntimeError(f"blue_slaver_suite_scope_drift:{record['case_id']}")

        backend = NativePublicBlueSlaverRolloutV1(state, sts)
        ...
        if simple is None or random_action is None:
            raise RuntimeError(f"blue_slaver_baseline_missing_action:{record['case_id']}")
        ...
        by_intent[intent]["cases"] += 1
        for key in counts:
            by_intent[intent][key] += int(agreement[key])
    ...
    summary = {
        "schema_version": SCHEMA,
        "scope": "opening-turn Blue Slaver V1 quality slice only",
        "phase1_gate_claimed": False,
        "case_count": total,
        "opening_intent_breakdown": by_intent,
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("blue_slaver_suite_freeze_contract_failed")

    rows1, summary1 = run_once(suite)
    rows2, summary2 = run_once(suite)
    if rows1 != rows2 or summary1 != summary2:
        raise RuntimeError("public_search_v1_blue_slaver_quality_not_deterministic")
    ...
    print("PUBLIC_SEARCH_V1_BLUE_SLAVER_QUALITY_EXECUTION = PASS")
    print(f"BLUE_SLAVER_QUALITY_VERDICT = {final_summary['quality_verdict']}")
