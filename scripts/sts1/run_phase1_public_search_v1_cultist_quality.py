#!/usr/bin/env python3
"""Run the frozen opening-Cultist quality benchmark for native PublicStateSearch V1.

This is a second narrow quality slice, not the full Phase-1 gate. Formal Search
receives public state only. The diagnostic oracle may use hidden simulator state
inside independently reconstructed sample contexts, but its answers are never
fed back into Search or suite selection.
"""
...
SCHEMA = "sts1-phase1-public-search-v1-cultist-quality-v1"
EXPECTED_SUITE_SCHEMA = "sts1-phase1-public-search-v1-cultist-quality-suite-v1"
...
def oracle_scores_for_case(context, backend, config) -> dict[str, float]:
    ...
    for sample_index in range(ORACLE_SAMPLE_COUNT):
        ...
        for semantic_key in sorted(semantic_groups):
            ...
            # Fresh reconstructed context for every candidate under the same
            # paired public sample prevents cross-action mutation.
...
def run_once(suite: dict) -> tuple[list[dict], dict]:
    ...
    for record in suite["cases"]:
        ...
        if context.decision_signature != record["decision_signature"]:
            raise RuntimeError(f"cultist_suite_signature_drift:{record['case_id']}")
        enemy = state["enemies"][0]
        if state.get("turn") != 0 or enemy.get("name") != "CULTIST" or enemy.get("intent") != "CULTIST_INCANTATION":
            raise RuntimeError(f"cultist_suite_scope_drift:{record['case_id']}")
        ...
        if simple is None or random_action is None:
            raise RuntimeError(f"cultist_baseline_missing_action:{record['case_id']}")
    ...
    summary = {
        "schema_version": SCHEMA,
        "scope": "opening-turn Cultist V1 quality slice only",
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("cultist_suite_freeze_contract_failed")

    rows1, summary1 = run_once(suite)
    rows2, summary2 = run_once(suite)
    if rows1 != rows2 or summary1 != summary2:
        raise RuntimeError("public_search_v1_cultist_quality_not_deterministic")
    ...
    final_summary.update({"full_repeats": 2, "deterministic": True, "suite_digest": suite["suite_digest"]})
    ...
    print("PUBLIC_SEARCH_V1_CULTIST_QUALITY_EXECUTION = PASS")
    print(f"CULTIST_QUALITY_VERDICT = {final_summary['quality_verdict']}")
