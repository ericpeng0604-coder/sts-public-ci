#!/usr/bin/env python3
"""Run the frozen opening-Gremlin-Nob quality benchmark for PublicStateSearch V1."""
...
SCHEMA = "sts1-phase1-public-search-v1-gremlin-nob-quality-v1"
EXPECTED_SUITE_SCHEMA = "sts1-phase1-public-search-v1-gremlin-nob-quality-suite-v1"
...
def run_once(suite: dict) -> tuple[list[dict], dict]:
    ...
    for record in suite["cases"]:
        ...
        if context.decision_signature != record["decision_signature"]:
            raise RuntimeError(f"gremlin_nob_suite_signature_drift:{record['case_id']}")
        enemy = state["enemies"][0]
        if state.get("turn") != 0 or enemy.get("name") != "GREMLIN_NOB" or enemy.get("intent") != "GREMLIN_NOB_BELLOW":
            raise RuntimeError(f"gremlin_nob_suite_scope_drift:{record['case_id']}")
        ...
        if simple is None or random_action is None:
            raise RuntimeError(f"gremlin_nob_baseline_missing_action:{record['case_id']}")
        ...
        rows.append(
            {
                ...
                "search_candidate_scores": [
                    {"action_id": item.action_id, "semantic_key": item.semantic_key, "score": item.score, "samples": item.samples, "unresolved": item.unresolved}
    ...
    summary = {
        "schema_version": SCHEMA,
        "scope": "opening-turn Gremlin Nob V1 quality slice only",
...
def main() -> None:
    ...
    if suite.get("schema_version") != EXPECTED_SUITE_SCHEMA or suite.get("case_count") != 24 or suite.get("frozen_before_quality_results") is not True:
        raise RuntimeError("gremlin_nob_suite_freeze_contract_failed")

    rows1, summary1 = run_once(suite)
    rows2, summary2 = run_once(suite)
    if rows1 != rows2 or summary1 != summary2:
        raise RuntimeError("public_search_v1_gremlin_nob_quality_not_deterministic")
    ...
    print("PUBLIC_SEARCH_V1_GREMLIN_NOB_QUALITY_EXECUTION = PASS")
    print(f"GREMLIN_NOB_QUALITY_VERDICT = {final_summary['quality_verdict']}")
