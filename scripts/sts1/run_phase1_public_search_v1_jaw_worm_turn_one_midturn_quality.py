#!/usr/bin/env python3
"""Run frozen quality benchmark for audited Jaw Worm turn-1 midturn states."""
...
SCHEMA = "sts1-phase1-public-search-v1-jaw-worm-turn-one-midturn-quality-v1"
...
def run_once(suite: dict) -> tuple[list[dict], dict]:
    ...
    generation_card_counts: Counter[str] = Counter()
    generation_card_agreement: dict[str, Counter[str]] = defaultdict(Counter)
    ...
    for record in suite["cases"]:
        ...
        played_card = str(record.get("generation_played_card", "UNKNOWN"))
        intent_counts[intent] += 1
        generation_card_counts[played_card] += 1
        ...
        for key in counts:
            ...
            generation_card_agreement[played_card][key] += int(agreement[key])
    ...
    generation_breakdown = {
        card: {
            "cases": generation_card_counts[card],
            "search": generation_card_agreement[card]["search"],
            "simple": generation_card_agreement[card]["simple"],
            "random": generation_card_agreement[card]["random"],
        }
        for card in sorted(generation_card_counts)
    }
    summary = {
        "schema_version": SCHEMA,
        "scope": "Jaw Worm second-player-turn audited one-card midturn public states only",
        ...
        "generation_played_card_breakdown": generation_breakdown,
...
def main() -> None:
    ...
    if suite.get("source_counter_history_exported") != 0:
        raise RuntimeError("suite_exported_counter_history")

    rows1, summary1 = run_once(suite)
    rows2, summary2 = run_once(suite)
    if rows1 != rows2 or summary1 != summary2:
        raise RuntimeError("jaw_worm_turn_one_midturn_quality_not_deterministic")
    ...
    print("PUBLIC_SEARCH_V1_JAW_WORM_TURN_ONE_MIDTURN_QUALITY_EXECUTION = PASS")
    print(f"JAW_WORM_TURN_ONE_MIDTURN_QUALITY_VERDICT = {final_summary['quality_verdict']}")
