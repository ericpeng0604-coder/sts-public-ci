#!/usr/bin/env python3
"""Evaluate frozen Red Slaver turn-0 suites with raw PublicStateSearch V1."""
...
from roguelike_ai.sts1_teacher.native_rollout import NativePublicRedSlaverRolloutV1
...
CANDIDATE_ID = "red-slaver-turn0-raw-public-search-v1"
ORACLE_MCTS_SIMS = 2000
ORACLE_SAMPLE_COUNT = 4
SEARCH_SAMPLE_COUNT = 8
SEARCH_SAMPLING_SEED = 20260830
EXPECTED_SCHEMAS = {
    "discovery": "sts1-phase1-red-slaver-turn0-discovery-suite-v1",
    "holdout": "sts1-phase1-red-slaver-turn0-fresh-holdout-suite-v1",
}
...
def run_once(suite: dict, suite_kind: str) -> tuple[list[dict], dict]:
    ...
    for record in suite["cases"]:
        ...
        if context.decision_signature != record["decision_signature"]:
            raise RuntimeError(f"red_slaver_suite_signature_drift:{record['case_id']}")
        enemy = state["enemies"][0]
        if (
            state.get("turn") != 0
            or enemy.get("name") != "RED_SLAVER"
            or enemy.get("intent") != "RED_SLAVER_STAB"
        ):
            raise RuntimeError(f"red_slaver_suite_scope_drift:{record['case_id']}")

        backend = NativePublicRedSlaverRolloutV1(state, sts)
        if backend.uses_hidden_information:
            raise RuntimeError("red_slaver_backend_hidden_information")
        ...
        if simple is None or random_action is None:
            raise RuntimeError(f"red_slaver_baseline_missing_action:{record['case_id']}")
        ...
        rows.append(
            {
                "schema_version": f"sts1-phase1-red-slaver-turn0-{suite_kind}-quality-v1",
    ...
    summary = {
        ...
        "scope": "Red Slaver turn-0 opening slice only",
        "candidate_id": CANDIDATE_ID,
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("red_slaver_suite_freeze_contract_failed")
    ...
    if rows1 != rows2 or summary1 != summary2:
        raise RuntimeError("red_slaver_quality_not_deterministic")
    ...
    print(f"RED_SLAVER_{args.suite_kind.upper()}_QUALITY_EXECUTION = PASS")
    print(f"RED_SLAVER_{args.suite_kind.upper()}_QUALITY_VERDICT = {final_summary['quality_verdict']}")
