#!/usr/bin/env python3
"""Paired fresh-seed A/B: frozen ArmG v1 vs learned ArmG v3 candidate."""
...
def main():
 ...
 for n in ("module-dir","armg-root","v1-weight","v3-weight","formal-seed-file","output-dir"): p.add_argument("--"+n,type=Path,required=True)
 ...
 for seed in seeds:
  pair={}
  for label,w in (("v1",a.v1_weight),("v3",a.v3_weight)):
   ...
   pair[label]=dict(r)
  rows.append({"seed":seed,**pair});print(f"ARMG_AB {len(rows)}/{len(seeds)} v1={pair['v1'].get('outcome')}:{pair['v1'].get('final_floor')} v3={pair['v3'].get('outcome')}:{pair['v3'].get('final_floor')}",flush=True)
 def stat(k):
 ...
 v1=stat("v1");v3=stat("v3")
 paired={"v3_only_wins":sum(x["v3"].get("outcome")=="victory" and x["v1"].get("outcome")!="victory" for x in rows),"v1_only_wins":sum(x["v1"].get("outcome")=="victory" and x["v3"].get("outcome")!="victory" for x in rows)}
 report={"schema_version":"sts1-armg-v3-paired-ab-v1","seed_source":"fresh_random_excluding_formal_50","seed_count":len(seeds),"rng_seed":a.rng_seed,"mcts_sims":a.mcts_sims,"v1":v1,"v3":v3,"paired":paired,"rows":rows}
 (a.output_dir/"armg-v3-ab-summary.json").write_text(json.dumps(report,indent=2,sort_keys=True)+"\n");print("ARMG_V3_AB_RESULT",json.dumps({k:report[k] for k in ("seed_count","v1","v3","paired")},sort_keys=True))
