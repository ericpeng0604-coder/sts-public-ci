#!/usr/bin/env python3
"""Native proof + deterministic Search smoke for Anger + Pommel composition."""
...
def admitted_context_and_seeds(state: dict, aux: dict) -> tuple[DecisionContext, dict, dict]:
    ...
    assert admitted_state["reconstruction"]["anger_pommel_composition_complete"] is True
...
def verify_search_smoke(aux: dict) -> None:
    ...
    print("ANGER_POMMEL_SEARCH_SMOKE_STATES = 5")
    print("ANGER_POMMEL_SEARCH_SMOKE_REPEATS_PER_STATE = 2")
    print(f"ANGER_POMMEL_SEARCH_SMOKE_TOTAL_FIRST_REPEAT_ROLLOUTS = {total_rollouts}")
    print("ANGER_POMMEL_SEARCH_SMOKE_DETERMINISM = PASS")
    print("ANGER_POMMEL_SEARCH_SMOKE_ILLEGAL = 0")
    print("ANGER_POMMEL_SEARCH_SMOKE_UNRESOLVED = 0")
    print("ANGER_POMMEL_SEARCH_SMOKE_HIDDEN_INFORMATION = 0")
...
def main() -> None:
    ...
    assert sum(card["name"] == "Pommel Strike" for card in state["discard_pile"]) == 1
    assert aux["cards_played_this_turn"] == 2
    assert aux["attacks_played_this_turn"] == 2
    assert admitted.decision_signature == DecisionContext.from_public_state(state).decision_signature

    # Snapshot arithmetic alone must still reject this richer composition.
    try:
        ...
        raise AssertionError("Anger+Pommel composition unexpectedly admitted without reconstruction aux")
    ...
    assert sum(card.name == "Pommel Strike" for card in bc.discard_pile) == 1
    ...
    try:
        ...
        assert "anger_pommel_aux_incomplete" in str(exc), str(exc)
    else:
        raise AssertionError("incomplete composition aux unexpectedly admitted")

    bad_counter = dict(aux)
    bad_counter["cards_played_this_turn"] = 1
    try:
        ...
        raise AssertionError("wrong composition counters unexpectedly admitted")

    verify_search_smoke(aux)

    print("ANGER_POMMEL_COMPOSITION_NATIVE = PASS")
    print("ANGER_GENERATED_COPY_PUBLIC_PILES = PASS")
    print("POMMEL_DRAW_SHAPE_PUBLIC_PILES = PASS")
    print("TWO_ATTACK_AUX_COUNTER_PATH = PASS")
